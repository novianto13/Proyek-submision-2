...
Original file is located at
    https://colab.research.google.com/drive/1UDEJ_swVo2HQHFdj_5FtgDbv74qyao5N
"""
...
def create_byage_df(df):
    byage_df = df.groupby(by="order_status").customer_id.nunique().reset_index()
    byage_df.rename(columns={
        "customer_id": "customer_count"
    }, inplace=True)
    byage_df['order_status'] = pd.Categorical(byage_df['order_status'], ["Finish", "Process", "cancel"])
...
sns.barplot(x="order_item_id", y="product_category_name", data=sum_order_items_df.sort_values(by="quantity_x", ascending=True).head(5), palette=colors, ax=ax[1])
...
def create_bystatus_df(df):
    ...
    bystatus_df['order_status'] = pd.Categorical(bystatus_df['order_status'], ["Finish", "Process", "cancel"])
...
with col2:
    total_revenue = format_currency(daily_orders_df['sum'].sum(), "AUD", locale='es_CO')
    st.metric("Total Revenue", value=total_revenue)
